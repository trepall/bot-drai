...
async def cherryteam(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = "Родненький мой мигрантик рады видеть в рядах TDTTEAM!!!\nМожешь привязать своего мамонта по айди или юзеру."
    await update.message.reply_text(welcome_text, reply_markup=reply_markup)

async def handle_mammoth_binding(update: Update, context: ContextTypes.DEFAULT_TYPE, identifier: str):
    user_id = update.effective_user.id
    ...
    try:
        ...
        if mammoth_id not in mammoth_messages:
        ...
        await update.message.reply_text("❌ Ошибка при привязке мамонта.")
...
async def show_my_mammoths(query, user_id: int):
...
async def show_mammoth_messages(query, worker_id: int, mammoth_id: int):
    ...
    for msg in mammoth_messages[mammoth_id][-10:]:  # Последние 10 сообщений
