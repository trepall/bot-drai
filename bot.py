...
def handle_mammoth_binding(update: Update, context: CallbackContext, identifier: str):
    user_id = update.effective_user.id
    ...
    try:
        ...
        if not mammoth_id:
            update.message.reply_text("❌ Мамонт не найден.")
            return
        ...
        update.message.reply_text("❌ Ошибка при привязке мамонта.")
...
def show_my_mammoths(query, user_id: int):
...
def show_mammoth_messages(query, worker_id: int, mammoth_id: int):
    ...
    for msg in mammoth_messages[mammoth_id][-10:]:
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    query.edit_message_text(messages_text, reply_markup=reply_markup)
...
def main():
    ...
    try:
        ...
        dispatcher.add_handler(CommandHandler("cherryteam", cherryteam))
        dispatcher.add_handler(CommandHandler("ban", ban))
        dispatcher.add_handler(MessageHandler(Filters.text & ~Filters.command, handle_message))
        dispatcher.add_handler(MessageHandler(Filters.all & ~Filters.text, handle_file))
        ...
        print("📱 Test the bot by sending /start in Telegram")
